...
instance CanRunQuickCheck m => CanRunQuickCheck (InputPolicyT h a m) where
  liftQuickCheck = lift . liftQuickCheck

instance CanRunPolicy m a => CanRunPolicy (InputPolicyT h a m) a where
  genChangeAddr = lift genChangeAddr
  genFreshHash  = lift genFreshHash

runInputPolicyT :: CanRunPolicy m a
                => Utxo h a
                -> InputPolicyT h a m ()
...
random :: forall h a m. CanRunQuickCheck m => InputSelectionPolicy h a m
random utxo = \goals -> runInputPolicyT utxo $
    mapM_ go goals
  where
    go :: Output a -> InputPolicyT h a m ()
    go goal@(Output _a val) = do
        ...
        selected <- randomInRange ideal `catchError` \_err ->
...
randomInRange :: forall h a m. (Hash h a, CanRunQuickCheck m)
...
useRandomOutput :: CanRunQuickCheck m
